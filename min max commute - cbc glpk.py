...
scriptpath = 'C:\\Program Files\\Python38\\Lib\\site-packages\\tims_sandbox\\one_file_programs\\'
...
def commuter_evaluation(ags):
    ...
    if c_max[4] == 1:
        ...
        results.append(0)
    # random commute: converge, residents, workplaces, zone_index
...
def calculate_random_commute(converge, np_costs, residents, workplaces, zone_index, ags):
    # calculate random commute with proportional fitting of random commute table until
    # change of average of results goes below converge value [0.010 = 1.0%]
    log('Random Commute:')
    zones = sorted(zone_index[0].keys())
    zone2index = zone_index[0]
    index2zone = zone_index[1]
    ...
    # loop:
    while abs(c_rnd_old-c_rnd_new)/c_rnd_old > converge:
        rnd_seed = numpy.random.randint(100, size=(matrix_size,matrix_size))
        IPF = ipfn.ipfn(rnd_seed, aggregates, dimensions)
        rnd_commute_matrix = IPF.iteration()
##        # adjust result
